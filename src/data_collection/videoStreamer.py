...
def run(rpi_name, rpi_ip, usb):
    ...
    if usb:
        size = (480, 640)
    out = cv2.VideoWriter(str(file_name),fourcc, 17, size)
        ...
        try:
            ...
            if a!=-1 and b!=-1:
                ...
                if hour != current_hour:
                    ...
                    file_name = absolute_link + rpi_name + '_' + file_name + '.avi'
                    out = cv2.VideoWriter(file_name,fourcc,17, size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u','--usb',default='False')
    args = parser.parse_args()

    rpi_name = args.name
    rpi_ip = args.ip
    usb = args.usb
    run(rpi_name, rpi_ip, usb)
